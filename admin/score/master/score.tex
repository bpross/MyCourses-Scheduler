...
\begin{enumerate}\item	The Program Administrator signs into the browser-based MyCourses Portal with their credentials\item	Once logged in, the Program Administrator enters the Portal and a notice pops up indicating that the database needs information to be filled in order to continue\item	Upon opening the database, it opens an empty table in a spreadsheet format\item	The top of the table requires will need the following information: 
	\begin{enumerate}	\item	List of all classrooms and buildings available		\begin{enumerate}	
		\item Classroom size		\item Subject preference (Social Sciences belong with Sociology)
		\end{enumerate}	\item	List of all the day slots 	\item	List of time slots	\item	List of courses offered of every subject 
		\begin{enumerate}		\item	This will be submitted separately into a database table by the Program Manager
		\end{enumerate}	\item	Initially, each of the classrooms will be completely free and available unless otherwise 
	\end{enumerate}\item	The Program Administrator manually enters each classroom organized by building
	\begin{enumerate}	\item	Baskin Engineering $\to$ BE 156 	\item	E2 $\to$ E2 192
	\end{enumerate}\item	The Program Administrator also enters the time slots and dates available to all the classrooms	\begin{enumerate}
	\item	Monday, Wednesday and Friday 	\item	Tuesday \& Thursday	\item	Monday \& Wednesday
	\end{enumerate}\item	The Program Administrator enters all the courses organized by subject	\begin{enumerate}
	\item	Economics $\to$ Econ 1	\item	Computer Science $\to$ CMPS 12A
	\end{enumerate}\item	Once the information has been filled, the Program Administrator submits and the information will be stored and the Program Manager can start determining which courses to offer for the upcoming quarter
...
\begin{enumerate}\item	The Program Manager signs into the browser-based MyCourses Portal with their credentials 
	\begin{enumerate}	\item	This particular Program Manager is in charge of the Computer Science department.
	\end{enumerate}\item	Once logged in, a message pops up saying that the database is ready for information about the Fall 2010 quarter. The Program Manager opens a database of all listed courses for the Computer Science department specified by the Program Administrator
	\begin{enumerate}
	\item	This database will list all of the courses (in this case Computer Science courses) offered at UCSC.
