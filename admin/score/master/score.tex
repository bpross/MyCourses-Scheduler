...
\item User Interface
	\begin{enumerate}
	\item Web-based, multiple browser compatible:
		\begin{enumerate}
		\item Must meet HTML5 Validation Standard.
	...
	\item Views based on user roles:
		...
		\item Program Administrator:
		    ...
		    \item Must be able to perform operations on the database.
		    \item Must be able to run the algorithm.
		    \item Must be able to send and view messages from program manager.
		    \end{enumerate}
		\item Program Manager:
			\begin{enumerate}
		   \item Must be able to log in and out.
		   \item Must be able to retrieve population information set by program administrator.
		...
		\item Lecturer:
			\begin{enumerate}
			\item Must be able to log in and out.
			\item Must be able to view available times from program manager.
			\item Must be able to choose constraints.
			\end{enumerate}
		\item Student:
			\begin{enumerate}
		   \item Must be able to log in and out.
		   \item Must be able to choose courses.
		   \item Must be able to edit, modify, swap, or delete courses.
		   \end{enumerate}
		\end{enumerate}
	\item Must be able to connect through the connecting component to the database and the scheduler.
	\end{enumerate}
\item Database
	\begin{enumerate}
	\item A proper database system:
	      \begin{enumerate}
	      \item Must let us be able to store and retrieve data efficiently and reliably.
	      \item Must let us continuously store and retrieve data per logged in user session.
	      \end{enumerate}
	\item Must be able to connect through the connecting component to both the scheduler and the user interface.
	\end{enumerate}
\item Scheduler
      \begin{enumerate}
      \item Constraint-based algorithm:
	    ...
	    \item Must be able to provide an output that is easy to modify, should the algorithm not provide a solution for the given inputs.
	    \end{enumerate}
     \item Must be able to connect with the connecting component to display necessary information to the user interface.
     \end{enumerate}
\item Connecting Component
      \begin{enumerate}
      \item Interfacing between the user interface, the database, and the scheduler:
      	    \begin{enumerate}
	    \item Must be scalable to allow for systems both small and large.
	    \item	Must work well with database-driven websites.
	    \end{enumerate}
     \end{enumerate}
\end{enumerate}
...
The program administrator's user interface is fairly simple, they are given fields such as courses, rooms, and professors to fill out, once they fill out the fields to the proper specifications for the quarter then they will submit the data to the program manager. Once the program manager has completed their portion of the work the program administrator wil have an option to run the Scheduler. The Scheduler will run, and the results will be available to the rest of the users.

The program manager's interface will display all of the courses and lecturers for their department. The program manager will have the option of selecting the lecturers and courses that they want to be active for the quarter and will have a submit option, which will submit their choices to the program administrator.
